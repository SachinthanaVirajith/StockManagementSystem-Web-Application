    ...
    public partial class StockHistory : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (material == "All")
                {
                    cmd.CommandText = "select billId,material,wash,sellerName,date,fullWeight from material where date >'" + startDate + "' and date < '" + endDate + "'";
                }
                else
                {
                    cmd.CommandText = "select billId,material,wash,sellerName,date,fullWeight from material where material='" + material + "' and wash='" + washStatus + "' and date>'" + startDate + "' and date < '" + endDate + "'";
                }
                cmd.Connection = cnn;

                SqlDataReader rd = cmd.ExecuteReader();
                table.Append("<table border='0' width=1200px");
                table.Append("<tr><th>Bill Number</th><th>Material</th> <th>Wash Status</th> <th>Seller Name</th> <th>Date</th> <th>Full Weight</th>");
                table.Append("<tr>");
